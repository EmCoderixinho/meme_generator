...
const MemeEditor: React.FC = () => {
    ...
    const handleConfigChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        const value = e.currentTarget.type === 'checkbox' && e.currentTarget instanceof HTMLInputElement
            ? e.currentTarget.checked
            : e.currentTarget.value;

        setConfig(prev => ({
            ...prev,
            [name]: typeof value === 'string' && !isNaN(Number(value)) && value !== ''
                ? Number(value)
                : value,
        }));
    };
    ...
    // This new effect saves the configuration and gets an ID
    useEffect(() => {
        // Prevent saving the config on the initial component mount
    ...
    // This effect now uses the configId to fetch the preview
    useEffect(() => {
        if (!originalImage || configId === null) return;
    ...
    const handleGenerateMeme = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/meme/generate`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ image: originalImage, config }),
            });
