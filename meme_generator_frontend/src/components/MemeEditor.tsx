...
const MemeEditor: React.FC = () => {
  ...
  useEffect(() => {
    if (!originalImage) {
      setPreviewImage(null); // Clear preview if image is removed
      return;
    }
    ...
    const fetchPreview = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/meme/preview`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
  ...
  const handleGenerateMeme = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/meme/generate`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const renderControl = (
    ...
    <div className="mb-4">
      ...
      {type === 'range' && (
        <span className="text-sm ml-2">{config[name]}px</span>
      )}
    </div>
  );
    ...
    <div className="container mx-auto p-4 grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div className="md:col-span-1 bg-white p-6 rounded-lg shadow-lg">
        ...
        {renderControl('Font Family', 'fontFamily', 'select', {
          items: ['Impact', 'Arial', 'Helvetica', 'Comic Sans MS'],
        })}
        ...
        {renderControl('Text Align', 'textAlign', 'select', {
        ...
        <button
          ...
          className="w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded 
                     focus:outline-none focus:shadow-outline disabled:bg-gray-400 mb-4"
        >
        ...
        <button
          ...
          className="w-full bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded 
                     focus:outline-none focus:shadow-outline disabled:bg-gray-400"
        >
