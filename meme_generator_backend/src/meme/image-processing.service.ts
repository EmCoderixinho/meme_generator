...
export class ImageProcessingService {
   ...
   */
  bufferToBase64(buffer: Buffer, format: string = 'jpeg'): string {
   ...
   */
  async validateImage(imageBuffer: Buffer): Promise<{ isValid: boolean; error?: string }> {
    try {
      const metadata = await this.getImageMetadata(imageBuffer);
      ...
      return { isValid: false, error: `Invalid image: ${error.message}` };
    }
  }
   ...
   */
  getQualitySettings(useCase: 'preview' | 'final' | 'thumbnail'): { quality: number; format: string } {
    switch (useCase) {
      case 'preview':
        return { quality: 60, format: 'jpeg' };
      case 'thumbnail':
        return { quality: 80, format: 'jpeg' };
      case 'final':
        return { quality: 95, format: 'jpeg' };
      default:
        return { quality: 95, format: 'jpeg' };
    }
  }
